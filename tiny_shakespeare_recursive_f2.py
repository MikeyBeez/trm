...
# Now collect activations after training is complete - save in chunks
print("\nCollecting activations from trained model...")
model.eval()
num_batches = 500
chunk_size = 50  # Save every 50 batches to avoid OOM
...
with torch.no_grad():
    for i in range(num_batches):
        ...
        # Save chunk to disk and clear memory
        if (i + 1) % chunk_size == 0 or i == num_batches - 1:
            print(f"Saving chunk at batch {i+1}...")
            chunk_acts = torch.cat(model.penultimate_activations, dim=0).numpy()
            chunk_targets = torch.cat(model.ground_truths, dim=0).numpy()
            
            all_activations.append(chunk_acts)
            all_targets.append(chunk_targets)
            
            # Clear the lists to free memory
            model.penultimate_activations = []
            model.ground_truths = []

print("\nCombining all chunks...")
activations_array = np.concatenate(all_activations, axis=0)
ground_truths_array = np.concatenate(all_targets, axis=0)

print(f"Saving {activations_array.shape[0]} activation samples...")
