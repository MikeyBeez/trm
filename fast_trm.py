...
def main():
    ...
    # Train baseline
    print("\n" + "="*80)
    baseline = BaselineModel(config).to(config.device)
    print(f"Baseline parameters: {sum(p.numel() for p in baseline.parameters()):,}")
    baseline_results = train_baseline(baseline, train_dataset, val_dataset, config)
    ...
    print("FINAL RESULTS")
    print("="*80)
    print(f"Baseline best perplexity: {min(baseline_results['val_perplexity']):.2f}")
    print(f"TRM best perplexity: {min(trm_results['val_perplexity']):.2f}")
    print(f"Improvement: {(1 - min(trm_results['val_perplexity'])/min(baseline_results['val_perplexity']))*100:.1f}%")
